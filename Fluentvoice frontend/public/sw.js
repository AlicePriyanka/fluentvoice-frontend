...
// Dynamically compute the basePath from the service worker location
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith(`${basePath}/_next/static/`)) {
    event.respondWith(cacheFirst(request, STATIC_CACHE));
    return;
  }

  // Images and icons — cache-first
  if (url.pathname.startsWith(`${basePath}/icons/`) || url.pathname.match(/\.(png|ico|svg|webp|jpg)$/)) {
    event.respondWith(cacheFirst(request, STATIC_CACHE));
    return;
  }
...
async function networkFirstWithOfflineFallback(request) {
  try {
    ...
    const offline = await caches.match(`${basePath}/offline/`);
